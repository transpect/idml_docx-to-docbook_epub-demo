...
<p:declare-step 
  ...
  <p:option name="series" select="'idml2epub_hub'" required="false"/> 
  ...
  <trdemo:paths name="paths"> 
    <p:with-option name="pipeline" select="'idml2epub_hub.xpl'"/> 
    ...
    <p:input port="conf"> 
      <p:pipe port="conf" step="idml2epub_hub"/> 
    </p:input>
  </trdemo:paths>
 ...
 <p:delete match="@srcpath" name="delete-srcpath-inhierarchized-hub"/> 
  ...
  <bc:empty-report name="create-empty-report">
    <p:with-option name="pipeline" select="'idml2epub_hub'"/>
  </bc:empty-report>
  ...
  <letex:validate-with-rng-PI name="rng2pi">
    ...
    <p:with-option name="debug-dir-uri" select="$debug-dir-uri"/>
    <p:input port="schema">
      <p:pipe port="schema" step="idml2epub_hub"/>
    </p:input>
    <p:input port="source">
      <p:pipe port="result" step="evolve-hub-dyn"/>
    </p:input>
